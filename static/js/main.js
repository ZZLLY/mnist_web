        ...
        img.onload = () => {
            ...
            $.ajax({
                ...
                success: (data) => {
                    ...
                    for(var i=0;i<(data["site"][0]["output3"]).length;i++)
                    {
                        value_3[i] = Math.round(data["site"][0]["output3"][i] * 100);
                    }
                    // 获取最大值

                    var max_1 = Math.max.apply(Math,value_1);
                    var max_2 = Math.max.apply(Math,value_2);
                    var max_3 = Math.max.apply(Math,value_3);
                    var max_index_1 = value_1.indexOf(max_1);
                    var max_index_2 = value_2.indexOf(max_2);
                    var max_index_3 = value_3.indexOf(max_3);
                    for (let j=0;j<10;j++)
                    {
                    ...
                    for (let j=0;j<10;j++)
                    {
                         $('#result tr').eq(j + 1).find('td').eq(3).text(value_3[j]+"%");
                         if(j===max_index_3){
                             $('#result tr').eq(max_index_3+1).find('td').eq(3).addClass('success');
                         }
                    }
